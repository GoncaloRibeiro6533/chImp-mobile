...
class MockInvitationService(private val repoMock: RepoMock) :InvitationService {
    private suspend fun <T: Any>interceptRequest(token: String, block: suspend (User) -> Either<ApiError, T>): Either<ApiError, T> {
        val session = repoMock.userRepoMock.findSessionByToken(token) ?: return Either.Left(ApiError("Unauthorized"))
    ...
    override suspend fun createChannelInvitation(
        ...
        interceptRequest<Invitation>(senderToken) { sender ->
            val receiver = repoMock.userRepoMock.findUserById(receiverId) ?: return@interceptRequest Either.Left(ApiError("Receiver not found"))
            val channel = repoMock.channelRepoMock.findChannelById(channelId) ?: return@interceptRequest Either.Left(ApiError("Channel not found"))
            val invitation = repoMock.invitationRepoMock.createChannelInvitation(sender, receiver, channel, role)
    ...
    override suspend fun getInvitationsOfUser(token: String): Either<ApiError, List<Invitation>> =
        interceptRequest<List<Invitation>>(token) { user ->
            return@interceptRequest success(repoMock.invitationRepoMock.getInvitationsOfUser(user.id))
        }
    ...
    override suspend fun acceptInvitation(
        ...
        interceptRequest<Channel>(token) { user ->
            val invitation = repoMock.invitationRepoMock.acceptInvitation(invitationId)
            val inv = repoMock.invitationRepoMock.findInvitationById(invitationId) ?: return@interceptRequest failure(ApiError("Invitation not found"))
            ChannelRepoMock.userInChannel.add(UserInChannel(user.id, invitation.id, role = inv.role))
            return@interceptRequest success(invitation)
        }
    ...
    override suspend fun declineInvitation(
        ...
        interceptRequest<Boolean>(token) {
            return@interceptRequest success(repoMock.invitationRepoMock.declineInvitation(invitationId))
        }
